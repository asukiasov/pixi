...
| GitHub About blurb — revise to include "demo"/"preview" | ❌ open | Neither word appears in the current blurb; both are real search terms ("pixel art editor demo") and neither is a hype word repo-voice.md would object to. Suggested: "Browser-based pixel art editor — try the live demo, no install. Draw, preview, and export pixel art in your browser." (manual edit via repo Settings) |
| GitHub Social Preview image | ❌ unset | Confirmed via the repo's `og:image` — GitHub is serving its auto-generated fallback card (avatar + description + stats), not a screenshot. Upload `docs/screen.png` at Settings → General → Social preview. It's 1500×1229 against GitHub's recommended 1280×640 (2:1) — GitHub will center-crop it, which is acceptable (loses some of the browser chrome top/bottom, keeps the canvas), but a purpose-cropped 1280×640 version would look better if worth the effort |
