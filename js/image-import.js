...
// the Brush editor's Import (Pro-only, split-pixi-pro-repo - thresholds
// the result into a monochrome silhouette via pixi-pro's
// js/pro/brush-import.js), the Color Library panel's Import
// (2n-color-library-image-import, clusters the result into a palette via
// js/color-extraction.js - also Pro-only, not yet extracted as of this
// comment), and the reference image layer (reference-image-layer,
// js/layers.js's addReferenceImageLayer - part of Layers, also Pro-only,
// not yet extracted). Needs a real <canvas>/Image/createImageBitmap, like
...
// This file stays in pixi only until its last remaining consumer above
// is extracted - at that point it moves to pixi-pro wholesale, since it
// has no Standard-only caller. Thresholding/clustering logic built on
// top of this stays in its own module per feature (js/color-extraction.js
// here; js/pro/brush-import.js in pixi-pro) - this file only decodes,
...
// hasTransparency (alpha-vs-brightness threshold choice) moved to
// pixi-pro (split-pixi-pro-repo) alongside its only caller, the Brush
// editor's Import - see that repo's js/pro/brush-import.js.
