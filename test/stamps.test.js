...
import { STAMPS, placeStamp } from '../js/stamps.js';

describe('STAMPS registry', () => {
  test('includes a Heart stamp with a well-formed pattern', () => {
...
describe('placeStamp', () => {
  test('centers the pattern on the given point', () => {
    ...
    const stamp = { id: 'square', name: 'Square', width: 3, height: 3, pixels: [
      [0, 0], [1, 0], [2, 0],
      [0, 1], [1, 1], [2, 1],
      [0, 2], [1, 2], [2, 2],
    ] };
    placeStamp(engine, 10, 10, stamp, [255, 0, 0, 255]);
    // Center of a 3x3 stamp at (10,10): top-left = (10-1, 10-1) = (9,9)
    for (let y = 9; y <= 11; y++) {
      for (let x = 9; x <= 11; x++) {
  ...
  test('only sets "on" pixels, leaves "off" pixels of the pattern untouched', () => {
    ...
    const stamp = { id: 'diag', name: 'Diag', width: 2, height: 2, pixels: [[0, 0], [1, 1]] };
    placeStamp(engine, 5, 5, stamp, [0, 0, 0, 255]);
  ...
  test('clips the out-of-bounds portion silently near an edge', () => {
    ...
    assert.doesNotThrow(() => placeStamp(engine, 0, 0, stamp, [1, 2, 3, 255]));
