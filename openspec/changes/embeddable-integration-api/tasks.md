...
- [x] 3.2 Create `lib/pixi.js` exposing `Pixi.mount(hostElement, options)`, returning an instance with `destroy()` (the `#screen-workspace` markup `initWorkspace`/`initExport` need is an inline template string in `lib/pixi.js`, a deliberate copy of `index.html`'s block rather than a fetched partial or programmatic DOM build — no build step, self-contained/vendorable, per an explicit discussion with the user; drift risk documented inline as a tradeoff, no automated check yet. `mount()` starts a mounted instance on a blank `options.width`×`options.height` canvas, default 32×32, matching `js/new-canvas.js`'s own default — `loadImage`/`getImage`, task 3.3/3.4, aren't built yet. Found and fixed a real bug along the way: `saveProject(id, ...)` — called by every auto-save after a committed draw action, and a mounted instance has no project id until a host saves one — threw on `id == null` instead of no-op'ing like its doc comment already promised, because the Dexie adapter's `load()` forwards straight to `Table.get()`, which throws on null/undefined; fixed in `js/persistence.js`'s `saveProject`, covered by two new `test/persistence.test.js` cases. `validateHostElement` (the one pure-logic piece of `mount()`) is unit-tested in `lib/pixi.test.js` via TDD; the DOM/CanvasView/LayerStack-heavy rest of `mount()`/`destroy()` has no jsdom harness available (matches `test/theme.test.js`'s precedent) and was instead verified with a Playwright smoke test: mount into a host div, draw a stroke, Undo enables/works, `destroy()` empties the host, zero console errors. `npm test` 209/209)
