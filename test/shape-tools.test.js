...
import { drawRectangle, clipToSelection } from '../js/shape-tools.js';

describe('drawRectangle', () => {
  test('outline mode draws only the perimeter', () => {
    const engine = new PixelEngine(6, 6, 'transparent');
    drawRectangle(engine, 1, 1, 4, 4, [255, 0, 0, 255], false);
    // Corners and edges
  ...
  test('filled mode sets every pixel in the bounding box', () => {
    const engine = new PixelEngine(6, 6, 'transparent');
    drawRectangle(engine, 1, 1, 3, 3, [0, 255, 0, 255], true);
    for (let y = 1; y <= 3; y++) {
      for (let x = 1; x <= 3; x++) {
  ...
  test('normalizes corners regardless of drag direction', () => {
    ...
    drawRectangle(engineA, 4, 4, 1, 1, [1, 1, 1, 255], true);
    const engineB = new PixelEngine(6, 6, 'transparent');
    drawRectangle(engineB, 1, 1, 4, 4, [1, 1, 1, 255], true);
  ...
  test('a single-point rectangle (0-size drag) draws one pixel', () => {
    const engine = new PixelEngine(6, 6, 'transparent');
    drawRectangle(engine, 2, 2, 2, 2, [9, 9, 9, 255], false);
