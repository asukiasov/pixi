...
import { PixelEngine, circleOffsets, strokeFreehandThick, setPixelPerfectEnabled } from './engine.js';
...
describe('strokeFreehand', () => {
  ...
  test('keeps the corner pixel of an L-shaped path when pixel-perfect mode is off (default)', () => {
    // Corner-removal algorithm itself is covered in isolation by
    // pixel-perfect.test.js. This just confirms the default (pixel-perfect
    // off) leaves the corner pixel (1,0) untouched.
    const e = new PixelEngine(5, 5, 'transparent');
  ...
  test('drops the redundant corner pixel of an L-shaped path when pixel-perfect mode is on', () => {
    setPixelPerfectEnabled(true);
    try {
      const e = new PixelEngine(5, 5, 'transparent');
      e.strokeFreehand([{ x: 0, y: 0 }, { x: 1, y: 0 }, { x: 1, y: 1 }], [255, 0, 0, 255]);
      assert.deepEqual(px(e, 0, 0), [255, 0, 0, 255]);
      assert.deepEqual(px(e, 1, 0), [0, 0, 0, 0]); // corner pixel dropped
      assert.deepEqual(px(e, 1, 1), [255, 0, 0, 255]);
    } finally {
      setPixelPerfectEnabled(false); // don't leak into other tests
    }
  });
...
describe('setPixelBlended', () => {
  ...
  test('out-of-bounds coordinates are a no-op', () => {
    const e = new PixelEngine(2, 2, 'white');
    assert.doesNotThrow(() => e.setPixelBlended(-1, 0, [1, 2, 3, 255], 0.5));
    assert.doesNotThrow(() => e.setPixelBlended(5, 5, [1, 2, 3, 255], 0.5));
  });
});
...
describe('erasePixelBlended', () => {
  ...
  test('out-of-bounds coordinates are a no-op', () => {
    const e = new PixelEngine(2, 2, 'white');
    assert.doesNotThrow(() => e.erasePixelBlended(-1, 0, 0.5));
  });
});
