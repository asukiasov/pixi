...
- **WHEN** a buyer pays $5 via PayPal
...
host (Cloudflare Pages) connected directly to the private repo.
...
- **THEN** they get the complete, unrestricted Pro app (all Standard +
  ...
  collaborator gets, hosted on Cloudflare Pages rather than GitHub Pages
...
- **WHEN** the `pixi-pro` Cloudflare Pages demo goes live
- **THEN** `pixi`'s public README links to it alongside the existing
  Standard demo link, so visitors to the free repo can find and try Pro
