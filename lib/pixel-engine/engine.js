...
export class PixelEngine {
   ...
   * stroke. If pixel-perfect mode is on (see setPixelPerfectEnabled),
   * removeRedundantCorners runs on the raw path before pixels are set,
   * Aseprite-style.
   */
  strokeFreehand(points, rgba) {
    if (points.length === 0) return;
    const rawPath = interpolatePath(points);
    const finalPath = pixelPerfectEnabled ? removeRedundantCorners(rawPath) : rawPath;
   ...
   * setPixel. Used by the Pencil tool's Opacity setting (js/pencil-
   * opacity-ui.js).
   */
   ...
   * *color* via setPixelBlended would barely affect an opaque pixel at low
   * source alpha, not the same thing). RGB is preserved unless the result
   * is fully transparent, in which case it's zeroed to match the plain
   * eraser's [0, 0, 0, 0] convention. Used by the Eraser tool's Opacity
   * setting (js/pencil-opacity-ui.js).
   */
...
/**
 * Pixel-perfect drawing toggle (js/pixel-perfect-ui.js wires the
 * #pixel-perfect-toggle button to this directly - restored from the
 * Standard/Pro split, no extension-hook indirection). When on,
 * removeRedundantCorners runs on strokeFreehand/strokeFreehandThick's
 * interpolated path before pixels are set, Aseprite-style.
 */
let pixelPerfectEnabled = false;
export function setPixelPerfectEnabled(enabled) {
  pixelPerfectEnabled = enabled;
}
 ...
 * setPixelBlended/erasePixelBlended). Pixel-perfect mode (see
 * setPixelPerfectEnabled) only applies at size 1 - corner removal has no
...
export function strokeFreehandThick(points, size, applyPixel) {
  if (points.length === 0) return;
  const rawPath = interpolatePath(points);
  const finalPath = size === 1 && pixelPerfectEnabled ? removeRedundantCorners(rawPath) : rawPath;
