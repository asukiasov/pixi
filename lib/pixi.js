 ...
 * logged rather than propagated, so one bad listener can't break another
 * handler on the same event, or the commit/autosave it was invoked from.
 */
/**
    ...
    emit(event, ...args) {
      ...
      for (const handler of handlers) {
        try {
          ...
          console.error(`instance.on('${event}', ...) handler threw:`, err);
        }
      }
    },
  };
...
export const Pixi = {
  ...
  mount(hostElement, options = {}) {
    ...
    // Fire-and-forget, matching autoSave()'s own fire-and-forget calling
    // convention (js/workspace.js) - not awaited, not wrapped in a mount()-
    // level try/catch, so a rejection (e.g. a broken host-supplied adapter)
    // surfaces the same way an autoSave() failure already would: as an
    // unhandled rejection, not a mount()-time throw.
    createProjectWithId(projectId, layerStack, options.name ?? 'Untitled');
    ...
    // leak is a real, previously-hit failure mode in this file).
    ...
    return {
      /**
       * Registers `handler` to be called on `event` (task 3.5). Only
       * `'change'` is emitted today - fired from js/workspace.js's
       * autoSave() choke point on every committed drawing action (a
       * completed stroke, a fill, an undo/redo - anything that already
       * triggers auto-save), matching the "Change notifications" spec
       * scenario ("the host-registered change handler is invoked" when a
       * stroke commits). Multiple handlers on the same event are all
       * called, in registration order; see createEventEmitter's doc
       * comment for why there's no off()/unsubscribe. Registering after
       ...
       * A host's own storage layer (task 3.9, not built yet) committing
       * something durable beyond IndexedDB is a separate, later concern -
       * `cancel()` only ever touches the mounted canvas's own in-memory/
       * IndexedDB state, the same surface every other instance method
       * here already touches.
       *
